...
- [ ] **PROD-CRITIQUE — le build prod compilé ne démarre PAS** (découvert au smoke T6, 2026-06-01) : `node dist/server/server/index.js` échoue (`ERR_MODULE_NOT_FOUND` sur `import './app'`). Cause **pré-existante** (commit `ad95909`) : tout le source serveur utilise des **imports relatifs sans extension** + l'alias **`@shared/*`** non réécrit ; Node ESM (`type: module`) exige les `.js` explicites et ne connaît pas l'alias tsconfig. **Le dev marche (tsx tolère), la prod jamais testée.** Fix : passer `build:server` à un bundler (esbuild/tsup) OU `tsc` + `tsc-alias` + `moduleResolution`/extensions `.js`. **Doit aussi rendre `parse-child.js` lançable en standalone** (entrée séparée spawnée par le runtime isolé). Bloquant absolu avant tout déploiement.
