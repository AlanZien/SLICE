...
- [ ] **Câbler `pnpm prod:smoke` en CI pré-release** (avec `corpus-check`) : boote le binaire compilé + health + statique + fallback SPA + upload réel. C'est le garde-fou « vrai chemin de build » — sans CI il ne tourne pas automatiquement. (issu de fix-prod-build)
- [ ] **Figer la version pnpm** : `node_modules` installé en pnpm 10 (store v10), `pnpm` du PATH en 9.13.2 (store v3 incompatible → `pnpm add` casse). Ajouter `"packageManager": "pnpm@10.x"` dans `package.json` + activer corepack. (issu de fix-prod-build)
