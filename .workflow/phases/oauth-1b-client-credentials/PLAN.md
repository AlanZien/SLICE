...
- [ ] **T1 — Contexte + émission conditionnelle** : générateur expose tokenUrl/scopes/scopesJoined ; `oauth-token.ts` émis ssi oauth2. (couvert par snapshot T11 + runtime ; commit support.)
- [ ] **T2 — R10** : kit oauth2 env sans `UPSTREAM_OAUTH_CLIENT_ID/SECRET` → le serveur refuse de démarrer / erreur claire. RED runtime (serveur n'émet pas "listening on").
- [ ] **T3 — R11** : appel d'un tool → le serveur token mocké reçoit `POST`, `Authorization: Basic <base64(id:secret)>`, `Content-Type: x-www-form-urlencoded`, body `grant_type=client_credentials` (+ `scope=a b` si scopes). RED.
- [ ] **T4 — R12** : l'upstream mocké reçoit `Authorization: Bearer <access_token retourné par le token mock>`. RED.
- [ ] **T5 — R13** : `expires_in:3600` → 2 appels séquentiels → **1** POST token ; `expires_in:0` → 2 POST ; sans `expires_in` → 1 POST (défaut 300 s). RED.
- [ ] **T6 — R14** : module `oauth-token` importé isolément, `getAccessToken()` ×3 en parallèle → **1** fetch token (in-flight partagé). RED.
- [ ] **T7 — R15** : upstream renvoie 401 puis 200 → 1 refetch token + succès (2 POST token) ; 401 deux fois → erreur remontée (pas de boucle). RED.
- [ ] **T8 — R16/R16bis** : token endpoint 500 → erreur claire ; réponse `token_type:'mac'` → erreur. RED.
- [ ] **T9 — R17/R17bis** : aucun fichier généré ne contient la valeur du secret ; forcer une erreur token → message sans secret ni base64 ni body réponse. RED.
- [ ] **T10 — R18** : `.env.example` oauth2 contient `UPSTREAM_OAUTH_CLIENT_ID`/`UPSTREAM_OAUTH_CLIENT_SECRET`. RED (assertion contenu).
- [ ] **T11 — R19** : étendre `mcp-generator.snapshot.test.ts` — bundle oauth2 `tsc --noEmit` OK + `oauth-token.ts` présent. RED.
- [ ] **T12 — R20** : kit oauth2 en `MCP_AUTH_MODE=relay` → token de l'agent relayé en Bearer à l'upstream, **aucun** POST tokenUrl. RED (réutilise le banc relai).
