...
Statut : GENERATE terminé (T1-T8, 444 tests verts) → EVALUATE
...
- [x] `src/shared/types.ts` — `EndpointParam.in` += `'body'` ; `EndpointParam.schema?: ZodSchemaShape` ; **`EndpointParam.wireName?: string`** (nom amont réel quand la clé exposée est désambiguïsée — R-B7).
- [x] `src/server/services/zod-schema-builder.ts` — `toZodShape()` + extension `ZodSchemaShape` (`additionalProperties`) + objets `.passthrough()` (string `buildZodExpression`).
- [x] `src/server/services/spec-normalizer.ts` — lit `op.requestBody`, aplatit (+`description` des props), collision→`wireName`, fallback.
- [x] `src/server/services/hosted-mcp-factory.ts` — sélecteur `buildZodSchema` (body→`param.schema`) ; `callUpstream` réassemble + envoie le body.
- [x] `src/server/services/mcp-generator.ts` — body params dans l'inputSchema (via `param.schema`) + `hasBody`/`bodyFallback` + `bodyExpr` (quoting via helpers existants).
- [x] `src/server/templates/tools.ts.hbs` — branche étalé `body: { {{bodyExpr}} }` vs fallback `body: {{access 'body'}}`.
- [x] `src/server/services/spec-to-hosted-config.ts` — propagation (copie déjà `params` ; test de garde + `description`/`wireName` survivent).

## Tâches (ordre TDD, dépendances respectées)

- [x] **T1 — Modèle** : `EndpointParam.in += 'body'` + `schema?: ZodSchemaShape` + **`wireName?: string`**. Type-only (pas de test propre), débloque le reste.
- [x] **T2 — Builders permissifs + parité** : étendre `ZodSchemaShape` (`additionalProperties`), objets `.passthrough()` dans `buildZodExpression` (string) **et** `buildZodSchema` (runtime). **3 sous-cas canoniques** : object+props, object sans props (free-form), additionalProperties.
...
- [x] **T3 — `toZodShape(openApiSchema)`** : fonction pure de conversion.
...
- [x] **T5 — Runtime hébergé envoie le body** :
...
- [x] **T6 — Kit généré envoie le body** :
...
- [x] **T7 — Cas Notion-like hébergé (E2E)** : `{}` et un objet à **clés dynamiques** (`properties`) transmis **intacts** (R-B3) ; couvre R-B6 (descriptions exposées).
- [x] **T8 — Propagation config hébergée** : test de garde `spec-to-hosted-config` — les params `in:'body'` + `schema` + `wireName` + `description` arrivent dans `HostedMcpConfig`.
...
## Definition of Done
- [x] T1–T8 cochées, tests TDD verts (parité kit↔hébergé + cas Notion-like).
- [x] `pnpm typecheck` clean, suite verte.
- [x] EVALUATE **STANDARD** (pas de chemin auth/secret nouveau ; le body = donnée non-sensible, le relai existe déjà) : `/simplify`.
- [x] PR sur `feature/request-body-forwarding`, UAT.md + RETRO mis à jour.
