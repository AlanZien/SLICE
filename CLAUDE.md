...
**OAuth amont — chantier en cours (SPEC validée : `.workflow/SPEC-OAUTH-UPSTREAM.md`)**. Cadrage : flow auto = `client_credentials` self-host (secrets en env de l'utilisateur) ; SLICE Cloud ne stocke jamais de secret (hébergé = relai d'un Bearer) ; `authorization_code`/login navigateur = phase ultérieure. Découpé en 4 sous-phases (1a→{1b‖1c}→1d).
- **OAuth-1a livré (PR #30, mergé 2026-06-02)** : SLICE n'écarte plus les specs OAuth2/OIDC. Détection du flow `client_credentials` → variant `oauth2 {tokenUrl, scopes}` propagé jusqu'à la config ; autres flows + OIDC → `bearer` ; basic/digest seuls restent rejetés. Filtrage par schémas **référencés** (helper partagé `collectReferencedSchemeNames`). **Aucun code généré/runtime touché.** Détail : `.workflow/phases/oauth-1a-detection/`.
- **Prochaine : OAuth-1b** (cœur risqué — obtention/cache/refresh/retry du token self-host). **Commencer par un spike** (banc de test double-mock token+upstream). Règles R10-R19bis de la SPEC. LEARN OAuth à faire à la fin (après 1d).
